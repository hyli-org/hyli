    ...
    async fn query_sequenced_txs(
        ...
        let rows = sqlx::query(
            ...
            WITH contract_txs AS (
                SELECT DISTINCT t.parent_dp_hash, t.tx_hash, t.block_hash, t.transaction_status, t.block_height, t.index, t.lane_id, t.identity, blk.timestamp
                FROM transactions t
                JOIN blobs b
            ...
            SELECT ct.parent_dp_hash, ct.tx_hash, ct.index, ct.lane_id, ct.identity, ct.timestamp, ct.block_hash, ct.transaction_status, ct.block_height, b.blob_index, b.data, b.contract_name
