...
impl IntoHeaderSignableData for MempoolNetMessage {
    fn to_header_signable_data(&self) -> HeaderSignableData {
        match self {
            ...
            MempoolNetMessage::DataProposal(lane_id, hash, _, vote) => {
                HeaderSignableData(
...
impl Mempool {
    pub(super) fn on_data_vote(&mut self, lane_id: LaneId, vdag: ValidatorDAG) -> Result<()> {
        ...
        let signatures = match self.lanes.add_signatures(
            &lane_id,
            &data_proposal_hash,
            std::iter::once(vdag),
        ) {
    ...
    fn maybe_cache_poda(
        ...
        if metadata.cached_poda.is_some() {
            return Ok(());
        }
    ...
    fn send_net_message_broadcast(&mut self, net_message: MempoolNetMessage) -> Result<()> {
        ...
        let error_msg = format!("Broadcasting MempoolNetMessage::{enum_variant_name} msg on the bus");
        self.bus
