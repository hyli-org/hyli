...
COPY src ./src
COPY crates ./crates
RUN cargo build --bin hyle --bin indexer --bin hyle-loadtest --bin gcs_uploader --bin smt_auto_prover --bin nuke_tx --bin force_wallet_registration --release -F sp1 -F risc0
...
COPY --from=builder /usr/src/hyle/target/release/force_wallet_registration ./
